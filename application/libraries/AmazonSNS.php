...
class AmazonSNS {
	...
	public function publishToOne($target_arn, $message, $opt = null)
	{
		if (!$opt) $opt = array();
		$opt['TargetArn'] = $target_arn;
		$opt['MessageStructure'] = 'json';
		$opt['Message'] = $message;
	...
	public function createEndpoint($appArn, $token) {
		if(empty($appArn)) {
			throw new InvalidArgumentException('Must supply a AppArn to create an endpoint');
		}
		...
		$this->updateEndpoint(strval($resultXml->CreatePlatformEndpointResult->EndpointArn),$token);
	...
	public function updateEndpoint($arn,$token) {
		if(empty($arn)) {
			throw new InvalidArgumentException('Must supply a Token to create an endpoint');
		}
		try{
		...
		$resultXml = $this->_request('SetEndpointAttributes', $params);
		}
	...
	public function deleteEndpoint($arn) {
		...
		$params = array('EndpointArn' => $arn);

		$resultXml = $this->_request('DeleteEndpoint', $params);
		}
		catch(SNSException $e) {
            // Amazon SNS returned an error
        }
	...
	public function subscribe($topicArn, $protocol, $endpoint) {
		...
		return strval($resultXml->SubscribeResult->SubscriptionArn);
